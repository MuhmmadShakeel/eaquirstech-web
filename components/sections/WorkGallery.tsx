  ...
  return (
    <section className="section-y bg-[#f7f9fc]">
      <div className="container-site">
        ...
        {shownProjects.length ? <div className="mt-10 grid justify-items-center gap-8 sm:grid-cols-2">{shownProjects.map((project, index) => <ProjectCard key={project.slug} project={project} index={index} />)}</div> : <p className="py-16 text-center text-body">No projects match that search.</p>}
      </div>
    </section>
  );
}
