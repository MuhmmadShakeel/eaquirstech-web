    ...
    return (
      <a {...revealProps(index)} href={project.href} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.name} demo`} className="group mx-auto block h-[400px] w-[400px] max-w-full [perspective:1200px] focus:outline-none">
        <div className="relative h-full w-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
          <div className="absolute inset-0 overflow-hidden rounded-3xl bg-[#0d0d0d] [backface-visibility:hidden]">
            <img src={image} alt={`${project.name} platform preview`} className="h-full w-full object-cover" />
          </div>
          <div className="absolute inset-0 flex overflow-hidden rounded-3xl bg-[#000] p-6 [backface-visibility:hidden] [transform:rotateY(180deg)]">
            ...
            <div className="mt-4 border-t border-white/15 pt-3"><span className="text-micro font-bold uppercase tracking-label text-white/45">Technology stack</span><ul className="mt-2 flex flex-wrap gap-1.5">{project.stack.slice(0, 4).map((tech) => <li key={tech} className="rounded-md bg-white/10 px-2 py-1 text-micro text-white/75">{tech}</li>)}</ul></div>
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-white/15 pt-3">{project.metrics.slice(0, 2).map((metric) => <div key={metric.label} className="min-w-0"><span className="block text-body font-bold text-amber">{metric.value}</span><span className="block text-micro text-white/55">{metric.label}</span></div>)}</div></div>
          </div>
        </div>
      </a>
    );
  ...
  return (
    <article {...revealProps(index)} aria-label={`Project: ${project.name}`} className="group mx-auto h-full w-full max-w-[400px] overflow-hidden rounded-2xl bg-white shadow-card transition-all duration-base hover:-translate-y-1 hover:shadow-card-hover">
      <div className="relative aspect-[16/10] overflow-hidden bg-[#0d0d0d]">
        <img src={image} alt={`${project.name} platform preview`} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
        <span className="absolute left-4 top-4 rounded-full border border-white/30 bg-black/55 px-3 py-1.5 text-micro font-bold uppercase tracking-label text-white backdrop-blur-sm">{project.category}</span>
      </div>
      <div className="flex h-full flex-col p-6">
        ...
        <div className="mt-5 border-t border-line pt-4">
          <span className="text-micro font-bold uppercase tracking-label text-muted">Technology stack</span>
          <ul className="mt-2 flex flex-wrap gap-1.5">{project.stack.slice(0, 5).map((tech) => <li key={tech} className="rounded-md border border-line bg-[#f7f9fc] px-2 py-1 text-micro text-body">{tech}</li>)}</ul>
        </div>
        <div className="mt-5 grid grid-cols-2 gap-3 border-t border-line pt-4">{project.metrics.slice(0, 2).map((metric) => <div key={metric.label}><span className="block text-lead font-bold text-amber-deep">{metric.value}</span><span className="text-micro text-muted">{metric.label}</span></div>)}</div>
        {project.href && <Button href={project.href} external size="md" arrow className="mt-6 self-start bg-amber text-white hover:bg-amber-bright">View demo</Button>}
      </div>
    </article>
  );
}
