...
export default function ServicesHeroImage() {
  return (
    <div className="relative select-none">
      ...
      <div className="absolute -top-4 -left-4 hidden rounded-xl border border-amber/30 bg-dark-card/90 px-4 py-3 shadow-mega backdrop-blur-sm md:block">
        <p className="text-micro text-on-dark-muted">Fastest delivery</p>
        <p className="mt-0.5 text-[1.6rem] font-light leading-none text-amber">6 wks</p>
        <p className="mt-1 text-micro text-on-dark-muted">full system replacement</p>
      </div>

      {/* Floating badge — bottom right */}
      <div className="absolute -bottom-4 -right-4 hidden rounded-full border border-white/10 bg-dark-card/90 px-4 py-2.5 backdrop-blur-sm md:flex items-center gap-2.5">
        <span className="h-2 w-2 rounded-full bg-green-400 animate-pulse" />
        <span className="text-micro font-semibold text-white/80">Senior engineers only</span>
      </div>
    </div>
  );
}
